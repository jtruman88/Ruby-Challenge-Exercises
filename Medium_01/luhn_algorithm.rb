...
Use test suite from luhn_algorithm_test.rb
...
class Luhn
  ...
  def self.create(number)
    if self.new(number).valid?
      return number
    else
      number = (number.to_s << '0').to_i
      until self.new(number).valid?
        number += 1
      end
    end
    number
  ...
  def addends
    nums = num.to_s.chars.reverse.map.with_index do |num, ind|
      prod = num.to_i * 2
      if ind.odd?
        prod >= 10 ? prod - 9 : prod
      else
